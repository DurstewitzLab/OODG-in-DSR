...
function plot_reconstruction_2d(X::AbstractArray{T, 3}, X̃::AbstractArray{T, 3}) where {T}
    ...
    fig = @views plot(X[:, 1, 1], X[:, 2, 1], label = "true", legend = true, color = :black)
    @views plot!(
        fig,
        X̃[:, 1, 1],
        X̃[:, 2, 1],
    ...
    @views plot!(X[:, 1, 2:end], X[:, 2, 2:end], label = false, color = :black)
    @views plot!(fig, X̃[:, 1, 2:end], X̃[:, 2, 2:end], label = false, color = :orange)
    return fig
end
...
function plot_reconstruction(
    X_gen_cpu::AbstractArray{T, N},
    X_cpu::AbstractArray{T, N},
    save_path::String,
) where {T, N}
    if size(X_cpu, 2) == 3
    ...
    elseif size(X_cpu, 2) >= 3
        ...
        @views fig =
            plot_reconstruction_multiple_series(X_cpu[:, :, 1], X_gen_cpu[:, :, 1], n_plots)
    end
